...
public class SectorDialogController implements Initializable {

	@FXML ComboBox<Sector> currency;
	...
	public void initialize(URL location, ResourceBundle resources) {
		ListUtil.setupComboBox(EMFProperties.list(MY_FOND__SECTOR_LIST).observe(datasource.getRoot()), currency, EMFProperties.value(SECTOR__NAME));
			...
			public void run() {
				{
					...
					IObservableValue mObs = EMFProperties.value(SECTOR_DISTRIBUTION__SECTOR).observe(distribution);
					dbc.bindValue(uiObs, mObs);
				}
				
				{
					IObservableValue uiObs = JFXUIProperties.text().observe(percentage);
					IObservableValue mObs = EMFProperties.value(SECTOR_DISTRIBUTION__PERCENTAGE).observe(distribution);
					dbc.bindValue(uiObs, mObs);
				}				
			}
		});
