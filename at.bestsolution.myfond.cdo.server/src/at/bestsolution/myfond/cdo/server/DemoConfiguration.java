...
//	}
	...
	protected void doActivate() throws Exception {
		...
		props.put(IRepository.Props.SUPPORTING_AUDITS,
				mode == Mode.NORMAL ? "false" : "true");
		props.put(IRepository.Props.SUPPORTING_BRANCHES,
				mode == Mode.BRANCHING ? "true" : "false");

		repository = (InternalRepository) CDOServerUtil.createRepository(name,
				store, props);
...
//	}
	...
	protected IDBStore createStore() {
		final IMappingStrategy mappingStrategy = CDODBUtil
				.createHorizontalMappingStrategy(true, false, true);
		final IDBAdapter dbAdapter = new H2Adapter();
		final IDBConnectionProvider dbConnectionProvider = DBUtil
				.createConnectionProvider(createDataSource());
	...
	protected DataSource createDataSource() {
		...
		dataSource.setURL("jdbc:h2:/tmp/databases/" + name + "/h2test");
		return dataSource;
	}
	...
	protected InternalSessionManager createSessionManager() {
		...
		sessionManager.addListener(new ContainerEventAdapter<ISession>() {
			...
			protected void onAdded(IContainer<ISession> container,
					ISession session) {
				ISignalProtocol<?> protocol = (ISignalProtocol<?>) session
						.getProtocol();
				if (protocol != null) {
