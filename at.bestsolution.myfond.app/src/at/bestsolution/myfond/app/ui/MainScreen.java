...
public class MainScreen {
	...
	void init(MApplication app, BorderPane parent) {
		...
		
		{
			...
			{
				Label l = new Label("Fonds Admin",new ImageView(getClass().getClassLoader().getResource("icons/kspread.png").toExternalForm()));
			...
			{
				Label l = new Label("Basic Data",new ImageView(getClass().getClassLoader().getResource("icons/preferences-desktop.png").toExternalForm()));
				l.setContentDisplay(ContentDisplay.TOP);
				l.getStyleClass().add("item");
				GridLayoutPane.setConstraint(l, new GridData(Alignment.CENTER, Alignment.CENTER, false, true));
				box.getChildren().add(l);	
